	...
	def createQuestionnaireExportKey(Long questionnaireId) {
		...
		if (questionnaire) {
			...
			if ((role == UserGroup.Role.TEACHER) ||
				...
				def pendencyKey = new PendencyKey(entity: questionnaireId, type: PendencyKey.Type.QUIZZ_EXPORT)
						.save(flush: true)
	...
	def download(String key) {
		...
		if (pendencyKey) {
			if (pendencyKey.type == PendencyKey.Type.QUIZZ_EXPORT) {
				def questionnaire = Questionnaire.get(pendencyKey.entity)
